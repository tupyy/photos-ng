...
const SubAlbumsList: React.FC<SubAlbumsListProps> = ({ albums, loading = false }) => {
  ...
  if (loading) {
    return (
      <div className="mb-8">
        <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Sub-albums</h2>
    ...
    <div className="mb-8">
      ...
      <div className="flex flex-wrap gap-4">
        {sortedAlbums.map((album: AlbumType) => (
          <SubAlbumCard key={album.id} album={album} />
        ))}
      </div>
    </div>
  );
